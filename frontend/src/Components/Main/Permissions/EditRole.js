...
class EditRole extends Component {
  ...
  render() {
    const { classes, roles, name, department, email, primaryTitle, photoUrl } = this.props;

    const roleList = roles
      .sort((a, b) => a.rank > b.rank)
      .map((role) =>
      <FormControlLabel key={role.rank} value={role.name} control={<Radio />} label={role.name} />
    );

    return (
      <div>
        <Button onClick={this.handleClickOpen} variant="contained" className={classes.button}>Edit</Button>
        <Dialog
        ...
        >
          <DialogContent className={classes.content}>
            <div className={classes.about}>
              <Avatar alt="User" src={photoUrl} className={classNames(classes.avatar, classes.bigAvatar)} />
              <div className={classes.member}>
                <div className={classes.name}>{name}</div>
                <div>{department}</div>
                <div>{email}<span> ({primaryTitle})</span></div>
              </div>
            </div>
          ...
          <DialogActions className={classes.buttons}>
            <Button onClick={this.handleDelete} variant="contained" color="secondary" size="small" className={classes.button}>
              Delete
            ...
            <div>
              ...
              <Button onClick={this.handleSave} color="primary" className={classes.button}>
                Save
              </Button>
            </div>
          </DialogActions>
